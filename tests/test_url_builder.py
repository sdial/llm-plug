...
def test_build_upstream_url_uses_base_url_fallback_for_standard_chat_endpoint():
    ...
    assert build_upstream_url(channel.selected_endpoint()) == "https://api.openai.com/v1/chat/completions"
...
def test_build_upstream_url_uses_endpoint_url_advanced_override():
    ...
    assert build_upstream_url(channel.selected_endpoint()) == "https://gateway.example.com/custom/chat"


def test_build_upstream_url_falls_back_to_base_url_when_endpoint_url_is_blank():
    channel = Channel(
        name="Custom",
        endpoints=[Endpoint(api_type=APIType.OPENAI_RESPONSE, base_url="https://api.example.com/v1", url_override=" ")],
        api_key="sk-test",
    )

    assert build_upstream_url(channel.selected_endpoint()) == "https://api.example.com/v1/responses"
