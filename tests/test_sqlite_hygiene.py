...
def test_stats_source_no_64mb_cache_no_parse_decltypes_no_bare_with_connect():
    ...
    assert "cache_size=-64000" not in src, (
        "stats.py 不应使用 cache_size=-64000:短连接场景每次申请 64MB 是浪费,应使用 SQLite 默认 cache 配合 mmap_size 共享 OS page cache"
    )
    # PARSE_DECLTYPES 是无效配置(没有 DECLTYPE 列),删掉避免误导
    assert "PARSE_DECLTYPES" not in src, "stats.py 不应使用 PARSE_DECLTYPES:表里没有 TIMESTAMP/DATE 列声明,该 flag 无效"
    bare = _find_bare_connections(src)
    assert not bare, f"stats.py 出现 {len(bare)} 处不会 close 的连接获取 {bare!r};必须改成 `with closing(...) as conn, conn:` 模式"


def test_request_logs_source_no_64mb_cache_no_bare_with_connect():
    src = _read_source(request_logs)
    assert "cache_size=-64000" not in src, "request_logs.py 不应使用 cache_size=-64000:短连接场景每次申请 64MB 是浪费"
    bare = _find_bare_connections(src)
    assert not bare, f"request_logs.py 出现 {len(bare)} 处不会 close 的连接获取 {bare!r};必须改成 `with closing(...) as conn, conn:` 模式"


def test_storage_stats_source_no_bare_connect_no_month_db_layout_knowledge():
    """ADR-0017 D1 守门：storage_stats 是月度分库的纯视图，源码内不得出现
    裸连接 / glob 月库目录 / 月库文件名格式——行数统计、目录发现与路径构造
    必须转调 request_logs 后端的公开接口（统一连接工厂是唯一连接来源）。"""
    src = _read_source(storage_stats)
    bare = _find_bare_connections(src)
    assert not bare, f"storage_stats.py 出现 {len(bare)} 处不会 close 的连接获取 {bare!r};必须转调 request_logs 后端公开接口"
    assert "sqlite3.connect" not in src, "storage_stats.py 不得自行连接 SQLite:行数统计走 request_logs 后端 month_db_record_count(统一连接工厂)"
    assert "import glob" not in src and "glob.glob" not in src, "storage_stats.py 不得自行 glob 月库目录:目录发现走后端 discover_month_dbs"
    assert "request_logs_" not in src, "storage_stats.py 不得复制月库文件名格式 request_logs_{y}_{m}.sqlite3:路径构造走后端 month_db_path"
...
def _assert_robust_short_conn_pragmas(conn: sqlite3.Connection) -> None:
    ...
    assert cache_size != -64000, f"cache_size 不应为 -64000 (64MB);短连接场景应使用 SQLite 默认或更小值,got {cache_size}"
...
@pytest.mark.asyncio
async def test_request_logs_sqlite_connect_has_robust_short_conn_pragmas(
    request_logs_db,
):
    ...
    month_db = backend.month_db_path(current_ym)
