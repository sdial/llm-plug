...
class ToResponseConverter(BaseConverter):
    ...
    def _anthropic_request_to_response(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        if data.get("thinking") is not None:
            budget = (
                data["thinking"].get("budget_tokens", 0)
                if isinstance(data["thinking"], dict)
                else 0
            )
    ...
    def _chat_stream_chunk_to_response(
        ...
        if self._stream_state["response_id"] and not self._stream_state[
            "response_id"
        ].startswith("resp_"):
            self._stream_state["response_id"] = self._make_response_id(
                self._stream_state["response_id"]
            )
        if self._stream_state["message_id"] and not self._stream_state[
            "message_id"
        ].startswith("msg_"):
        ...
        if usage:
            self._stream_state["input_tokens"] = usage.get(
                "prompt_tokens", self._stream_state["input_tokens"]
            )
            self._stream_state["output_tokens"] = usage.get(
                "completion_tokens", self._stream_state["output_tokens"]
            )
            self._stream_state["total_tokens"] = usage.get(
        ...
        if delta.get("tool_calls"):
            ...
            for tc in delta["tool_calls"]:
                ...
                if not call_id and tc_index is not None:
                    call_id = self._stream_state["tool_call_index_to_id"].get(
                        tc_index, ""
                    )
                ...
                if function.get("name"):
                    ...
                    if not call_id:
                        call_id = (
                            ...
                            else f"call_{len(self._stream_state['tool_calls'])}"
                        )
                    if tc_index is not None:
                ...
                if function.get("arguments") is not None:
                    ...
                    if args:
                        ...
                        if tc_index is not None:
                            event["output_index"] = (
                                self._stream_state["tool_calls"]
                                .get(call_id, {})
                                .get("output_index", tc_index)
                            )
                        events.append(event)
            if events:
                if _ensure_created():
        ...
        if finish_reason is not None:
            ...
            logger.debug(
                f"[CHUNK] built {len(done_events)} final events, need_created={need_created}"
            )
            ...
            if len(done_events) == 1:
                logger.debug(
                    f"[CHUNK] returning single done_event: {done_events[0].get('type')}"
                )
                return done_events[0]
            logger.debug(
                f"[CHUNK] returning first of {len(done_events)} events, rest in pending"
            )
    ...
    def _anthropic_stream_chunk_to_response(
        self, chunk: dict[str, Any]
    ) -> dict[str, Any] | None:
        if self._stream_state is None:
            self._reset_stream_state()
        ...
        if event_type == "message_start":
            ...
            if delta.get("type") == "text_delta":
                ...
                if not self._stream_state["reasoning_started"]:
                    self._stream_state["reasoning_started"] = True
                    self._stream_state["reasoning_id"] = (
                        f"rs_{self._stream_state['message_id']}"
                    )
                    idx = self._stream_state["output_index"]
                    self._stream_state["output_index"] = idx + 1
                    block = {
                    ...
                    self._stream_state["anthropic_content_blocks"][
                        chunk.get("index", 0)
                    ] = block
                    result = {
                        "type": "response.output_item.added",
            ...
            final_usage = anthropic_to_openai_response(
                self._stream_state["anthropic_usage"]
            )
            return {
    ...
    def get_extra_events(self, chunk: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        logger.debug(
            f"[GET_EXTRA_EVENTS] returning {len(events)} events, types={[e.get('type') for e in events]}"
        )
    ...
    def finalize_stream(self, source_type: str = "") -> list[dict[str, Any]]:
        logger.debug(
            f"[FINALIZE] source_type={source_type} stream_state={self._stream_state is not None}"
        )
        ...
        if not self._stream_state["response_id"]:
            ...
            logger.debug(
                f"[FINALIZE] generated response_id={self._stream_state['response_id']}"
            )
        if not self._stream_state["message_id"]:
            self._stream_state["message_id"] = self._stream_state["response_id"]
        if self._stream_state["message_id"] and not self._stream_state[
    ...
    def _build_final_events(
        ...
        if self._stream_state.get("input_tokens_details"):
            usage_data["input_tokens_details"] = self._stream_state[
                "input_tokens_details"
            ]
        if self._stream_state.get("output_tokens_details"):
            usage_data["output_tokens_details"] = self._stream_state[
        ...
        if finish_reason == "length":
            completed_event["response"]["incomplete_details"] = {
    ...
    def _queue_final_events_for_finish(
        self, finish_reason: str
    ) -> list[dict[str, Any]]:
        final_events = self._build_final_events(
            finish_reason=finish_reason, mark_completed=False
        )
    ...
    def _release_pending_completed_event(self) -> list[dict[str, Any]]:
        ...
        if completed.get("type") == "response.completed":
            ...
            if self._stream_state.get("input_tokens_details"):
                usage["input_tokens_details"] = self._stream_state[
                    "input_tokens_details"
                ]
            if self._stream_state.get("output_tokens_details"):
                usage["output_tokens_details"] = self._stream_state[
                    "output_tokens_details"
                ]
