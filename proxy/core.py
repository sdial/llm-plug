...
_model_channels_lock = _channel_registry._model_channels_lock
_model_channels_sync_lock = _channel_registry._model_channels_sync_lock
_background_tasks = _channel_registry._background_tasks


def _sync_model_channel_state_to_registry() -> None:
    _channel_registry._model_channels_cache = globals().get("_model_channels_cache")
    _channel_registry._model_channels_cache_version = globals().get(
        "_model_channels_cache_version",
        _channel_registry._model_channels_cache_version,
    )


def _sync_model_channel_state_from_registry() -> None:
    globals()["_model_channels_cache"] = _channel_registry._model_channels_cache
    globals()["_model_channels_cache_version"] = (
        _channel_registry._model_channels_cache_version
    )


async def _get_channels_for_model(model: str) -> list[Channel]:
    _sync_model_channel_state_to_registry()
    try:
        return await _registry_get_channels_for_model(model)
    finally:
        _sync_model_channel_state_from_registry()


def _schedule_invalidate_model_channels_cache() -> None:
    _sync_model_channel_state_to_registry()
    try:
        _registry_schedule_invalidate_model_channels_cache()
    finally:
        _sync_model_channel_state_from_registry()


# Keep core module state synchronized when storage invokes the registry callback directly.
def _sync_model_channel_state_after_registry_callback() -> None:
    _sync_model_channel_state_from_registry()


CONVERTER_MAP = _conversion.CONVERTER_MAP


def _response_input_to_items(input_data: Any) -> list[dict[str, Any]]:
    return _conversion.response_input_to_items(input_data)
...
async def _prepare_openai_response_request_for_upstream(
    ...
    original_store = _conversion._responses_store
    _conversion._responses_store = globals().get("_responses_store", original_store)
    try:
        return await _conversion.prepare_openai_response_request_for_upstream(
            request_data, source_type, target_api_type
        )
    finally:
        _conversion._responses_store = original_store


def _filter_channels_by_conversion(
    channels: list[Channel], target_api_type: APIType
) -> list[Channel]:
    original_get_setting = _conversion.get_setting
    _conversion.get_setting = globals().get("get_setting", original_get_setting)
    try:
        return _conversion.filter_channels_by_conversion(channels, target_api_type)
    finally:
        _conversion.get_setting = original_get_setting


def _get_converter_and_upstream_type(
    channel: Channel, target_api_type: APIType
) -> tuple:
    original_map = _conversion.CONVERTER_MAP
    _conversion.CONVERTER_MAP = globals().get("CONVERTER_MAP", original_map)
    try:
        return _conversion.get_converter_and_upstream_type(channel, target_api_type)
    finally:
        _conversion.CONVERTER_MAP = original_map
