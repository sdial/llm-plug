...
- [x] **步骤 4：为 `metadata`、`safety_identifier`、`prompt_cache_key` 写测试**
...
- [x] **步骤 2：为历史中的 reasoning item 写测试**

如果 Chat 上游可表达 `reasoning_content`，保存并回放；否则保存时保留但回放时跳过并记录 debug log。

- [x] **步骤 3：修正 `_response_output_to_items()`**
...
- [x] **步骤 3：替换单一 `output_item_added_sent` 布尔状态**
